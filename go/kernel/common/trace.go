...
func (s Syscall) traceBasicArg(arg uint64, typ reflect.Type) string {
	switch typ {
...
func (s Syscall) traceArg(args []uint64, typ reflect.Type) string {
	if typ == BufType && len(args) > 1 {
		...
		return s.traceBasicArg(args[0], typ)
	}
}
...
func (s Syscall) TraceRet(args []uint64, ret uint64) {
	...
	if len(s.Out) > 0 {
		out = append(out, s.traceBasicArg(ret, s.Out[0]))
	}
	if len(out) > 0 {
